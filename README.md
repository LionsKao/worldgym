...
前端呼叫 `worker/` 部署出來的 API(`/queryClasses` 等端點),API 讀寫 Cloudflare D1(`worldgym-schedule` 資料庫)。`worker/wrangler.toml` 的 `[triggers]` 設了三個 cron:`*/5 * * * *` 每 5 分鐘掃一次課前推播是否到期(跟爬蟲無關);`0 19 * * *`、`0 9 * * *` 則是台灣時間 03:00、17:00 各自動重抓一次全部分店(需要 Workers Paid 方案,因為單次執行要把 subrequest 上限拉到 1000 才跑得完 104 家分店)。也可以直接在 `admin.html` 手動觸發重抓。
...
1. 前端透過 `sw.js` 註冊 Service Worker、跟瀏覽器要 Push 訂閱(`ensurePushSubscription`),連同上課資訊一起打 `worker` 的 `/registerReminder` 存進 D1。
...
首頁上方的廣告輪播內容存在 D1 的 `ads` table(`text` / `url` / `startAt` / `endAt` / `enabled` / `sortOrder`)。前端載入時打 `worker` 的 `GET /ads`,只會拿回目前在上下架時間內、且 `enabled = 1` 的廣告;拿不到任何廣告時,整條廣告列會自動隱藏。
...
`admin.html` 登入後有 3 個報表面板,都用 [Chart.js](https://www.chartjs.org/) 畫圖(vendor 進 `vendor/chart.umd.min.js`,不吃 CDN):
...
網站沒有帳號系統,所以「幾個人」這類統計(例如最愛建立人數)是靠前端在 `localStorage` 存一個 `crypto.randomUUID()` 產生的匿名 id(`wg_client_id`),打點時帶著這個 id 送到 worker,後端用 `COUNT(DISTINCT clientId)` 估算——這個 id 不對應任何真實身分,純粹統計用。

新增報表面板的大致流程:worker 開一個 `/trackXxx` 打點端點 + 一個 `/xxxStats` 給 admin 讀,新增對應的 D1 table(記得同步寫 migration),前端在對應的使用者動作發生時打 `/trackXxx`,admin.js 再用共用的 `createDualLineChart`(雙 Y 軸折線圖)畫出來。
...
- 前端用 `script.js` 的 `setupPublicRankingStats()` 共用一份邏輯,畫出三張排行長條圖(Chart.js),每張只顯示**前 10 名**(`maxItems: 10`)。
...
課表查詢是最耗 D1 讀取量的端點,公開給匿名訪客用,沒有登入機制,做不到真正的「私有 API」——只能拉高濫用成本,不追求 100% 擋住。兩層防護都在 `worker/src/index.js`:
...
# 部署前端(不要用 npm run deploy,那是舊的 Cloudflare Pages 部署方式)
cd hosting && npx wrangler deploy
```
